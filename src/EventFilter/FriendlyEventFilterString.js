...
const FriendlyEventFilterString = (props) => {
  const { children, eventFilter, reportCount, className } = props;
  ...
  return <span style={{lineHeight: 'normal'}} className={className || ''}>Showing {filterModified && 'filtered'} {reportCount} reports updated from <strong>{calcFriendlyDurationString(date_range.lower, date_range.upper)}</strong>{children}</span>;
};
