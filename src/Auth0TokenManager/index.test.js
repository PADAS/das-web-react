...
describe('Auth0TokenManager', () => {
  ...
  describe('silent token refresh', () => {
    test('silently refreshes token when authenticated but no token exists', async () => {
      const validToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.test.signature';
      mockGetAccessTokenSilently.mockResolvedValue(validToken);
    ...
    test('does not refresh token when token already exists', async () => {
      useAuth0.mockReturnValue({
        isAuthenticated: true,
        getAccessTokenSilently: mockGetAccessTokenSilently,
      });
      useSelector.mockImplementation((selector) => {
        const state = {
          data: { token: { access_token: 'existing_token' } },
          view: { systemConfig: { require_idp: true } }
        };
        return selector(state);
      });
    ...
    test('does not refresh token when IDP is not required', async () => {
      useAuth0.mockReturnValue({
        isAuthenticated: true,
        getAccessTokenSilently: mockGetAccessTokenSilently,
      });
    ...
    test('does not refresh token when not authenticated', async () => {
      useAuth0.mockReturnValue({
        isAuthenticated: false,
        getAccessTokenSilently: mockGetAccessTokenSilently,
      });
