...
const TracksLayer = (props) => {
  ...
  return <Fragment>
    {trackData.map(data => <TrackLayer key={`track-layer-${data.track.features[0].properties.id}`} map={map} onPointClick={onTimepointClick} showTimepoints={showTimepoints} trackData={data} />)}
  </Fragment>;
};
