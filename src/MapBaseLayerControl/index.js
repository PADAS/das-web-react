...
const BaseLayerControl = (props) => {
  ...
  return <div className={styles.wrapper} ref={wrapperRef}>
    ...
    <Overlay placement={ufaNavigationUIEnabled ? 'left' : 'right'} show={popoverOpen} rootClose onHide={() => setPopoverOpenState(false)} container={wrapperRef.current} target={wrapperRef.current}>
      <Popover className={styles.popup} title='Base Layers'>
        <ul className={ufaNavigationUIEnabled ? styles.layerList : styles.oldNavigationLayerList}>
