...
describe('SideBar - DetailViewHeader - Breadcrumb', () => {
  ...
  test('keeps the history state and replace flag of a crumb on its link', async () => {
    const LocationDisplay = () => <p>{`${useNavigationType()} ${JSON.stringify(useLocation().state)}`}</p>;

    render(<>
      <Breadcrumb
        aria-label="Event navigation"
        crumbs={[{ label: 'Light', replace: true, state: { temporalId: '1234' }, to: '/events/new' }, { label: 'Fence' }]}
      />

      <LocationDisplay />
    </>);

    await userEvent.click(screen.getByRole('link', { name: 'Light' }));

    expect(screen.getByText('REPLACE {"temporalId":"1234"}')).toBeInTheDocument();
  });
