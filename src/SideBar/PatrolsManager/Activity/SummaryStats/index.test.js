...
const EMPTY_TEAM_AND_TRACKING = { assets: [], members: [] };
...
describe('SideBar - PatrolsManager - Activity - SummaryStats', () => {
  const endedPatrol = {
    id: 'patrol-1',
    patrol_segments: [
      { ...EMPTY_TEAM_AND_TRACKING, id: 'leg-1', leader: RANGER, time_range: FIRST_LEG_TIME_RANGE },
      { ...EMPTY_TEAM_AND_TRACKING, id: 'leg-2', leader: DOG, time_range: SECOND_LEG_TIME_RANGE },
    ],
    state: 'open',
  };
  ...
  beforeEach(() => {
    ...
    store = {
      data: {
        patrolTeamAndTrackingOptions: TEAM_AND_TRACKING_OPTIONS,
        subjectStore: {},
        tracks: {
  ...
  test('shows a dash on every measurement of a leg due to begin later', () => {
    const legStartingLater = {
      ...EMPTY_TEAM_AND_TRACKING,
      id: 'leg-3',
      leader: RANGER,
      time_range: { end_time: null, start_time: '2099-04-14T01:00:00.000Z' },
    };
    const patrolWithLegAhead = {
      ...endedPatrol,
      patrol_segments: [...endedPatrol.patrol_segments, legStartingLater],
    };

    renderSummaryStats({ eventCount: 0, patrol: patrolWithLegAhead, patrolSegment: legStartingLater });
  ...
  test('adds up the distance a tracked subject covered across every leg it took part in', () => {
    const singleSubjectPatrol = {
      ...
      patrol_segments: [
        { ...EMPTY_TEAM_AND_TRACKING, id: 'leg-1', leader: RANGER, time_range: FIRST_LEG_TIME_RANGE },
        { ...EMPTY_TEAM_AND_TRACKING, id: 'leg-2', leader: RANGER, time_range: SECOND_LEG_TIME_RANGE },
      ],
    };
    store.data.tracks = {
      [RANGER.id]: {
  ...
  test('moves the focus among the tracked subjects that are left after the patrol changes', async () => {
    const threeLeggedPatrol = {
      ...
      patrol_segments: [
        ...
        { ...EMPTY_TEAM_AND_TRACKING, id: 'leg-3', leader: PILOT, time_range: THIRD_LEG_TIME_RANGE },
      ],
    };
  ...
  describe('given a leg', () => {
    test('measures the duration of that leg alone', () => {
      renderSummaryStats({ patrolSegment: endedPatrol.patrol_segments[1] });

      expect(readSummaryStats()['Duration']).toBe('1h 30m');
    });

    test('reports no paused time for a leg that ran, since a pause is a leg of its own', () => {
      renderSummaryStats({ patrolSegment: endedPatrol.patrol_segments[1] });

      expect(readSummaryStats()['Paused Time']).toBe('0m');
      expect(readSummaryStats()['Active Time']).toBe(readSummaryStats()['Duration']);
    });

    test('counts the whole of a pause as paused time, leaving it no active time', () => {
      const pause = { ...endedPatrol.patrol_segments[1], is_pause: true };

      renderSummaryStats({ patrol: { ...endedPatrol, patrol_segments: [endedPatrol.patrol_segments[0], pause] }, patrolSegment: pause });

      expect(readSummaryStats()['Paused Time']).toBe('1h 30m');
      expect(readSummaryStats()['Active Time']).toBe('0m');
    });

    test('keeps counting while that leg is still running', () => {
      renderSummaryStats({ patrol: activePatrol, patrolSegment: activePatrol.patrol_segments[1] });
    ...
    test('measures the distance covered on that leg alone', () => {
      renderSummaryStats({ patrolSegment: endedPatrol.patrol_segments[0] });

      expect(readSummaryStats()['Distance']).toBe('111.2km');
    });

    test('reports no elapsed time for a leg that never began', () => {
      renderSummaryStats({
        patrolSegment: {
          ...EMPTY_TEAM_AND_TRACKING,
          id: 'leg-3',
          leader: PILOT,
          scheduled_start: THIRD_LEG_TIME_RANGE.start_time,
        },
      });

      expect(readSummaryStats()['Duration']).toBe('-');
      expect(readSummaryStats()['Distance']).toBe('-');
    });
  });
});
