...
describe('SideBar - PatrolsManager - PatrolsFeed - Filters', () => {
  ...
  test('moves focus into the popover it opens, which sits outside the tab order', async () => {
    renderFilters();
  ...
  test('resets the filters, the search text and the date range at once', async () => {
    store.data.patrolFilter.status = ['active'];
  ...
  test('marks a trigger as active and names its state while its own filter is modified', () => {
    ...
    expect(screen.getByRole('button', { name: 'Filters, filters applied' }).className)
      .toEqual(expect.stringContaining('active'));
    expect(screen.getByRole('button', { name: 'Dates' }).className).not.toEqual(expect.stringContaining('active'));
  });
  ...
  test('names the dates trigger state while the date range is modified', () => {
    ...
    expect(screen.getByRole('button', { name: 'Dates, custom date range applied' }).className)
      .toEqual(expect.stringContaining('active'));
  });
});
