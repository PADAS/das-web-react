...
// A preset up to the present leaves the end open, so the feed keeps taking new
// events rather than stopping at midnight.
...
const DateRangePopover = ({ className = '', onClose, ref, trigger, ...otherProps }) => {
  ...
  const dateRange = useSelector((state) => state.data.eventFilter.filter.date_range);
  ...
  // A range with no end runs until now.
  ...
  const onChangeEndDateTime = (newEndDateTime) => {
    ...
    if (newEndDateTime.startsWith(`${EMPTY_DATE_VALUE}T`)) {
