...
const isEmptyFieldValue = (value) => value === undefined
  || value === null
  || value === ''
  || (Array.isArray(value) && value.length === 0);

// A field only changes when it holds something the saved event did not: an
// emptied empty field or a default rjsf filled in is still what was saved.
const calculateSchemaFieldsChanges = (reportField, reportSchemaProps, originalReport) => Object.entries(reportField)
  .reduce((accumulator, [reportFieldKey, reportFieldValue]) => {
    const savedValue = originalReport.event_details?.[reportFieldKey];
    const schemaProp = reportSchemaProps?.[reportFieldKey];
    const isStillEmpty = isEmptyFieldValue(reportFieldValue) && isEmptyFieldValue(savedValue);
    // rjsf fills a default only into a missing value or an emptied choice.
    const isAutoFilledDefault = reportFieldValue === schemaProp?.default
      && (savedValue === undefined || (savedValue === '' && !!schemaProp?.enum));

    return isStillEmpty || isAutoFilledDefault ? accumulator : { ...accumulator, [reportFieldKey]: reportFieldValue };
  }, {});
...
const ReportDetailView = ({
  ...
  const { t: tHeader } = useTranslation('reports', { keyPrefix: 'eventsManager.eventOverview.header' });
  ...
  const eventFromStore = eventStore[reportId];
  // A blank saved title reads as no title, so the header falls back to the type
  // without the form counting that as a change.
  const reportFromStore = useMemo(() => setOriginalTextToEventNotes(eventFromStore && !eventFromStore.title?.trim()
    ? { ...eventFromStore, title: null }
    : eventFromStore), [eventFromStore]);
  ...
  const { displayTitle, eventTypeTitle } = useReport(reportForm);
  ...
  const parentCrumbs = useMemo(
    () => formProps?.parentCrumbs ?? [{ label: tHeader('breadcrumbEventsLabel'), to: `/${TAB_KEYS.EVENTS}` }],
    [formProps?.parentCrumbs, tHeader]
  );
  ...
  const reportChanges = useMemo(() => {
    ...
    return formattedReportDiffs.reduce((accumulator, [key, reportField]) => {
      ...
      if (key === 'title' && calcTitleAndSubtitle(reportField, eventTypeTitle).title.trim()
        === calcTitleAndSubtitle(originalReport.title, eventTypeTitle).title.trim()) {
        return accumulator;
      }

      return { ...accumulator, [key]: reportField };
    }, {});
  }, [eventSchema, eventTypeTitle, originalReport, reportForm]);
  ...
  const hasUnsavedChanges = attachmentsToAdd.length > 0 || newNotesAdded || Object.keys(reportChanges).length > 0;

  // An event being added somewhere is discarded on leaving, even untouched.
  const shouldShowNavigationPrompt = !isSaving
    && !redirectTo
    && (isAddedReport || isPatrolAddedReport || hasUnsavedChanges);

  const shouldShowAddReportButton = !isAddedReport
    && !isPatrolAddedReport
    && !belongsToPatrol
    && !relationshipButtonDisabled
    && !isCollectionChild
    ...
    try {
      onSaveReport(undefined, false).then(async ([{ data: { data: thisReportSaved } }]) => {
        if (reportForm.is_collection) {
          ...
          onSaveSuccess({}, [
            `/${TAB_KEYS.EVENTS}/${collectionRefreshedResults.data.data.id}`,
            { replace: true },
  ...
  const addReportFormProps = useMemo(() => ({
    onSaveSuccess: onSaveAddedReport,
    // A self-navigation, so the added event closes through the prompt.
  ...
  }), [displayTitle, location.pathname, location.search, location.state, onSaveAddedReport, parentCrumbs]);
  ...
  // An added event closes through a self-navigation, so both go through the
  // prompt.
  ...
  return <div
    ...
    <Header
      // A save sends the form as it was, so nothing can change until it ends.
      inert={isSaving}
      isCommunity={isCommunity}
      isReadOnly={eventType?.readonly}
      isStateDirty={Object.hasOwn(reportChanges, 'state')}
      isTitleDirty={Object.hasOwn(reportChanges, 'title')}
      onChangeState={onReportStateChange}
      onChangeTitle={onChangeTitle}
      onSaveReport={onSaveReport}
      parentCrumbs={parentCrumbs}
      printableContentRef={printableContentRef}
      report={reportForm}
      savedState={originalReport.state}
    ...
    <div className={styles.body} inert={isSaving}>
    ...
    <Footer
      addReportFormProps={addReportFormProps}
      isAddedReport={isAddedReport}
      isCommunity={isCommunity}
      isSaveDisabled={!isNewReport && !hasUnsavedChanges}
      isSaving={isSaving}
      onAddAttachments={onAddAttachments}
      onAddNote={onAddNote}
      onAddReport={onAddReport}
      onCancel={isAddedReport || isPatrolAddedReport ? onClickCancelButton : null}
      onSave={onClickSaveButton}
      onSaveAndSetState={onClickSaveAndSetState}
      report={originalReport}
      shouldShowAddReportButton={shouldShowAddReportButton}
    />
  </div>;
};
