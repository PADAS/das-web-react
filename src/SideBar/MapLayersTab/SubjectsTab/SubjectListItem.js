...
const SubjectListItem = (props) => {
  ...
  return <Fragment>
    <p className={styles.itemTitle} data-testid='subject-item-name'>
      {subjectRadioImage && <img src={subjectRadioImage} alt={subject.name} />}
      <span> {subject.name} </span>
      {!isEmpty(defaultProperty) && <span className={styles.defaultProperty}>{`${defaultProperty.label}: ${defaultProperty.value} ${defaultProperty.units}`}</span>}
    </p>
    <SubjectControls showLabels={false} className={styles.controls} map={map} showTitles={false} subject={subject} showTrackButton={!isStaticTypeObject} showHeatmapButton={!isStaticTypeObject}/>
  </Fragment>;
};
