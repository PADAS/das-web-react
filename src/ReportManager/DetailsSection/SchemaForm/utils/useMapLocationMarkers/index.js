...
const useMapLocationMarkers = (eventId, eventLocation, onMarkerClick = null, hideLayers = false) => {
  ...
  // Since the event id must not change, we build and store the source ids in a ref to avoid memoization processing.
  const sourceIdsRef = useRef({
    markers: `${MARKERS_SOURCE_ID}-${eventId}`,
    markerConnectingLines: `${MARKER_CONNECTING_LINES_SOURCE_ID}-${eventId}`,
  });

  // Same with layer ids.
  const layerIdsRef = useRef({
    markers: `${MARKERS_LAYER_ID}-${eventId}`,
    markerConnectingLines: `${MARKER_CONNECTING_LINES_LAYER_ID}-${eventId}`,
    markerConnectinOutlines: `${MARKER_CONNECTING_OUTLINES_LAYER_ID}-${eventId}`,
  });
