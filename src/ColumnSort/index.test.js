...
describe('ColumnSort control', () => {
  ...
  describe('changing the sort property', () => {
    ...
    test('clicking a sort option selects a new sort property', async () => {
      ...
      await userEvent.click(sortOptions[2]);

      expect(onSortChange).toHaveBeenCalledWith([SORT_DIRECTION.up, SORT_OPTIONS[2]]);

    });

    test('it should not change the sort option if the user clicks on same option in the popover', async () => {
      const sortOptionsContainer = screen.getByTestId('sort-options');
      const sortOptions = within(sortOptionsContainer).getAllByRole('button');
    ...
    test('it should change the sort option if the user clicks in different option in the popover', async () => {
      const sortOptionsContainer = screen.getByTestId('sort-options');
      const sortOptions = within(sortOptionsContainer).getAllByRole('button');
  ...
  describe('changing the sort order', () => {
    ...
    test('clicking a order option selects a new sort order', async () => {
      ...
      await userEvent.click(orderOptions[0]);

      expect(onSortChange).toHaveBeenCalledWith([SORT_DIRECTION.down, SORT_OPTIONS[0]]);
    });

    test('it should not change the order option if the user clicks on same option in the popover', async () => {
      const orderOptionsContainer = screen.getByTestId('order-options');
      const orderOptions = within(orderOptionsContainer).getAllByRole('button');

      // by clicking on current selected value
      await userEvent.click(orderOptions[1]);
      expect(onSortChange).not.toHaveBeenCalled();
    ...
    test('positive sorting', async () => {
      const sortDirectionControl = screen.getByTestId('sort-direction-toggle');

      await userEvent.click(sortDirectionControl);
      expect(onSortChange).toHaveBeenCalledWith([SORT_DIRECTION.down, SORT_OPTIONS[0]]);
    });

    test('negative sorting', async () => {
