...
const AttachmentListItem = ({ attachment, cardsExpanded, onCollapse, onDelete, onExpand }, ref = null) => {
  ...
  const onShowImageFullScreen = useCallback((event) => {
    ...
    patrolTracker.track('View fullscreen image from activity section');
  ...
  }, [attachment.filename, attachment.url, currentImageSource, dispatch, patrolTracker]);

  const onClickDownloadIcon = useCallback(() => {
    downloadFileFromUrl(attachment.url, { filename: attachment.filename });

    patrolTracker.track('Download patrol attachment');
  }, [attachment.filename, attachment.url, patrolTracker]);
  ...
  if (attachment.file_type === 'image') {
    return <li className={isOpen ? styles.openItem : ''} ref={ref}>
      <div className={`${styles.itemRow} ${styles.collapseRow}`} onClick={isOpen ? onCollapse : onExpand}>
        ...
        <div className={styles.itemDetails}>
          ...
          <DateTime
            ...
            data-testid={`patrolDetailView-activitySection-dateTime-${attachment.id}`}
        ...
        <div className={styles.itemActionButtonContainer}>
          <ItemActionButton>
            {isOpen
              ? <ArrowUpSimpleIcon data-testid={`patrolDetailView-activitySection-arrowUp-${attachment.id}`} />
              : <ArrowDownSimpleIcon data-testid={`patrolDetailView-activitySection-arrowDown-${attachment.id}`} />}
          </ItemActionButton>
        </div>
      </div>
      ...
      <Collapse
        ...
        data-testid={`patrolDetailView-activitySection-collapse-${attachment.id}`}
        in={isOpen}
      >
        <div>
          <img
  ...
  return <li className={styles.itemRow} ref={ref}>
    ...
    <div className={styles.itemActionButtonContainer}>
      <ItemActionButton onClick={!isNew ? onClickDownloadIcon : onDelete} tooltip={!isNew ? 'Download' : 'Delete'}>
        {!isNew
          ? <DownloadArrowIcon data-testid={`patrolDetailView-activitySection-downloadArrow-${attachment.id}`} />
          : <TrashCanIcon
            data-testid={`patrolDetailView-activitySection-trashCan-${attachment.filename || attachment.name}`}
          />}
      </ItemActionButton>
    </div>
