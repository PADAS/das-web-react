...
describe('PatrolDetailView - ActivitySection - Note', () => {
  ...
  test('sets the name New note to a new added note', async () => {
    ...
    expect((await screen.findByTestId('patrolDetailView-activitySection-noteTitle-')).textContent).toBe('New note');
  });
  ...
  test('adds the text New note: before the note text if the note is not part of the patrol yet', async () => {
    ...
    expect((await screen.findByTestId('patrolDetailView-activitySection-noteTitle-note')).textContent)
      .toBe('New note: note');
  });
  ...
  test('does not add the text New note: if the note is saved in the patrol already', async () => {
    ...
    expect((await screen.findByTestId('patrolDetailView-activitySection-noteTitle-1234')).textContent).toBe('note');
  });
  ...
  test('shows the date time of the last update of the note if it is saved in the patrol already', async () => {
    ...
    expect((await screen.findByTestId('patrolDetailView-activitySection-dateTime-1234'))).toBeDefined();
  });
  ...
  test('does not show the date time if it is a new note', async () => {
    ...
    expect((await screen.queryByTestId('patrolDetailView-activitySection-dateTime-note'))).toBeNull();
  });
  ...
  test('user can delete a new note', async () => {
    ...
    const deleteButton = await screen.findByTestId('patrolDetailView-activitySection-deleteIcon-note');
  ...
  test('user can not delete an existing note', async () => {
    ...
    expect((await screen.queryByTestId('patrolDetailView-activitySection-deleteIcon-1234'))).toBeNull();
  });
  ...
  test('user can edit a note', async () => {
    ...
    const editButton = await screen.findByTestId('patrolDetailView-activitySection-editIcon-note');
  ...
  test('user can open the note collapsible', async () => {
    ...
    expect(onExpand).toHaveBeenCalledTimes(0);
    expect((await screen.findByTestId('patrolDetailView-activitySection-collapse-note'))).toHaveClass('collapse');
  ...
  test('user can close the note collapsible', async () => {
    ...
    expect((await screen.findByTestId('patrolDetailView-activitySection-collapse-note'))).toHaveClass('show');
  ...
  test('user can not type an empty space at the beginning of a note', async () => {
    ...
    const noteTextArea = await screen.findByRole('textbox');
    const editButton = await screen.findByTestId('patrolDetailView-activitySection-editIcon-');
    userEvent.click(editButton);
  ...
  test('edit button is disabled while editing a new empty note', async () => {
    ...
    expect((await screen.findByTestId('patrolDetailView-activitySection-editIcon-'))).toHaveClass('disabled');
  });
});
